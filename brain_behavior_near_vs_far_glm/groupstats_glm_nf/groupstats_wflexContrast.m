...
contrastfile     = strtrim(paralist.contrastfile);
...
%     session = num2str(subjects1(subcnt1,3));
    sub_stats1{subcnt1,1} = fullfile(project_dir, 'results', fmri_type, 'participants', subject, stats_folder);                             
end
if numgroup == 2
...
%       session = num2str(subjects2(subcnt2,3));
      sub_stats2{subcnt2,1} = fullfile(project_dir, 'results', fmri_type, 'participants', subject, stats_folder);    
   end
end
   ...
   if(numgroup == 1)               
    if ~isempty(reg_vec)
      ...
      for j=1:numsubg1
        ...
        file = fullfile(sub_stats1{j}, contrastfile);
     ...
     for j=1:numsubg2
       file = fullfile(sub_stats2{j}, contrastfile);  
